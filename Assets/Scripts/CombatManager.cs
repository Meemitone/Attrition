...
public class CombatManager : MonoBehaviour
{
    ...
    IEnumerator DamageCleanup()
    {
        ...
        for (int i = 0; i < held.Length; i++)
        {
            ...
            {
                damage *= 1.25f;
                Movements[cCount] = StartCoroutine(held[i].gameObject.GetComponent<moveCoroutine>().FullMotion(held[i].stats.target.gameObject.transform.position, oneSidedTime, Quaternion.Euler(0, 0, held[i].arrowSprite.transform.rotation.eulerAngles.z - 90), oneSidedTime));
                cCount++;
                StartCoroutine(DamageAfterTime(damage, held[i].stats.target, oneSidedTime + oneSidedTime)); //time to rotate + time to bonk (in this case oneSidedTime on both)
            }
            else
            {
                Movements[cCount] = StartCoroutine(DelayMotion(oneSidedTime * 2 + oneSidedTime * 2, held[i], -90));
                cCount++;
                StartCoroutine(DamageAfterTime(damage, held[i].stats.target, oneSidedTime * 2 + oneSidedTime * 2 + clashTime + clashTime)); //time to rotate + time to bonk (in this case clashTime on both) + time for onesided attacks in full
            }
        }
        held = enemies.GetComponentsInChildren<UnitHolder>();
        for (int i = 0; i < held.Length; i++)
    ...
    IEnumerator DamageAfterTime(float damage, UnitHolder target, float time)
    {
    ...
    IEnumerator DelayMotion(float delay, UnitHolder held, float tweak)
    {
        yield return new WaitForSeconds(delay);
        yield return StartCoroutine(held.gameObject.GetComponent<moveCoroutine>().FullMotion((held.stats.target.gameObject.transform.position - held.gameObject.transform.position) / 3 + held.gameObject.transform.position, clashTime, Quaternion.Euler(0, 0, held.arrowSprite.transform.rotation.eulerAngles.z + tweak), clashTime));
        yield return null;
    }
}
